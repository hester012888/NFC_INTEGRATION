...
一个面向xxx省博物馆的移动端互动探馆应用。观众通过 NFC 打卡点触发互动，完成每件文物专属的任务，收集印章与打卡照片，解锁成就，并生成电子纪念卡。支持实时后端同步（WebSocket + REST），多人可同时打卡并实时看到进度。
...
本项目为博物馆互动体验演示用途。展品图片、音频等素材版权归xxx省博物馆及相关权利人所有。